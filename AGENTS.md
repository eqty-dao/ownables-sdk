...
Stack: React + TypeScript app (Create React App + CRACO).

## Project Structure
- `src/` — React app source (TypeScript, components, services).
- `ownables/` — Example Ownable packages (Rust/wasm or static).
- `bin/` — Helper scripts.
- `craco.config.js`, `ownable-js.webpack.js` — Build config.

## Commands
- `npm i` — Install dependencies
- `npm start` — Dev server (runs `build:ownable.js` first, then `craco start`)
...
## Code Style
- TypeScript strict mode; React 18 function components and hooks.
- UI: MUI v5. Follow MUI component patterns.
- Formatting: Prettier defaults (2 spaces, semicolons). Fix ESLint warnings.
- Error handling: `try/catch` in `src/services/*`; surface errors via MUI Snackbar/Notistack.
...
- If editing build config, verify both `npm start` and `npm run build`.
- CRACO extends CRA without ejecting—avoid eject.
